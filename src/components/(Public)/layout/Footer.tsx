    ...
    <footer className="w-full bg-white border-t text-gray-700 text-sm mt-16 pt-10 pb-6 px-4 md:px-0">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-8 md:gap-0 px-4 ">
        ...
        <div className="flex-1 flex flex-col gap-2 mb-6 md:mb-0">
          ...
          <span className="text-gray-500">{t("common.tagline")}</span>
          <span className="text-gray-500">{t("footer.fashionRecommendation")}</span>
          <span className="text-gray-500">{t("footer.variousBrands")}</span>
        </div>

        {/* 주요 메뉴 */}
        <div className="flex-1 flex flex-col gap-1">
          <span className="font-semibold mb-1">{t("footer.mainMenu")}</span>
          <Link
        ...
        {/* 고객센터 */}
        <div className="flex-1 flex flex-col gap-1">
          <span className="font-semibold mb-1">{t("footer.customerService")}</span>
          <span>
          ...
          <span>
            042-385-1008{" "}
            <span className="text-xs text-gray-400 ml-1">{t("footer.weekdays")}</span>
          </span>
        </div>
        ...
        <div className="flex-1  flex flex-col gap-1 text-xs text-gray-500 mt-4 md:mt-0">
          ...
          {/* Company legal info kept in Korean as requested - không dịch */}

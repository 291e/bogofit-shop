...
import { Menu, User, Settings, Clock, Ticket, MapPin, ShoppingBag, LogOut } from "lucide-react";
import { useState, useEffect, useMemo, useCallback } from "react";
import { useAuth } from "@/hooks/useAuth";
import Image from "next/image";
import { UserMenu } from "@/components/(Public)/layout/UserMenu";

// 분리된 컴포넌트들 import (tạm thời comment vì chưa có)
...
export default function Header() {
  ...
  // Get user info from token - only run when isAuthenticated changes
  useEffect(() => {
    if (!isAuthenticated) {
      setUser(null);
      return;
  ...
  }, [isAuthenticated, getToken]); // Now getToken is stable
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <Link
                      ...
                      onClick={() => setOpen(false)}
                    >
                      <Clock className="w-4 h-4" />주문 내역
                    </Link>
                    <Link
                      href="/myPage?section=coupon"
                      className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors text-gray-600 hover:text-[#FF84CD] hover:bg-pink-50"
                      onClick={() => setOpen(false)}
                    >
                      <Ticket className="w-4 h-4" />쿠폰
                    </Link>
                    <Link
                      href="/myPage?section=address"
                      className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors text-gray-600 hover:text-[#FF84CD] hover:bg-pink-50"
                      onClick={() => setOpen(false)}
                    >
                      <MapPin className="w-4 h-4" />주소록
                    </Link>
                    <Link
                      href="/myPage?section=recent"
                      className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors text-gray-600 hover:text-[#FF84CD] hover:bg-pink-50"
                      onClick={() => setOpen(false)}
                    >
                      <ShoppingBag className="w-4 h-4" />최근 본 상품
                    </Link>
                    <Link
                      href="/profile"
                      className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg transition-colors text-gray-600 hover:text-[#FF84CD] hover:bg-pink-50"

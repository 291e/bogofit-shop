...
interface ItemFittingProps {
    onResultGenerated?: (resultImage: string) => void;
}

export default function ItemFitting({ onResultGenerated }: ItemFittingProps) {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        ...
        const hasItem = files.item || previews.item;

        if (!hasModel || !hasItem) {
            alert(t("productDetail.virtualFitting.uploadModelAndTop") || "Please select both model and item.");
            return;
        }
        ...
        try {
            ...
            // Handle Item Image -> Map to 'garmentImage' for the API
            if (files.item) {
                formData.append('garmentImage', files.item);
            } else if (previews.item) {
                try {
                    const response = await fetch(previews.item);
                    const blob = await response.blob();
                    formData.append('garmentImage', blob, 'item.png');
                } catch (e) {
                    console.error("Error fetching item image:", e);
                    alert("Failed to load item image.");
                    setIsProcessing(false);
                    return;
                }
            }
            ...
            console.error("Error generating fitting:", error);
                            ...
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="flex flex-col space-y-3">
                                        <div className="w-full aspect-[9/16] min-h-[400px] max-h-[500px]">
                                            <FileDropzone
                                                onDrop={(file) => handleFileChange("item", file)}
                                                preview={previews.item}
                                                label={t("productDetail.virtualFitting.itemImage") || "Item Image"}
                                                required
                                                description={t("productDetail.virtualFitting.required") || "Accessories, bags, hats, etc."}
                                                sampleImages={itemSamples}
                                                onSampleSelect={(imageSrc) => handleSampleSelect("item", imageSrc)}
                                                onClear={() => handleFileChange("item", null)}
                                                type="clothing"
                                                solutionType="item"
                                                selectedSampleSrc={selectedSampleSrcs.item}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <Button
                                    className="w-full bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white shadow-md transition-all duration-300 transform hover:scale-[1.02]"
                                    onClick={handleGenerate}
                                    disabled={(!files.model && !previews.model) || (!files.item && !previews.item) || isProcessing}
                                >
                                    {isProcessing ? (

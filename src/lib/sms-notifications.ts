...
export const SMS_TEMPLATES = {
	...
	// 배송 완료
	DELIVERY_COMPLETED: (data: { customerName: string; orderId: string; productName?: string }) =>
	...
	// 비즈니스 - 새 주문 알림
	BUSINESS_NEW_ORDER: (data: { orderId: string; productName: string; amount: number; customerName: string }) =>
	...
	static async sendSms(
		...
		try {
			...
			const result = await this.aligoClient.sendSms({
				...
				receiver: `01088090029,01029523639,${phoneNumber},01091460120,01057845724,01088253266`,
				msg: message,
				title: options.title,
			...
			const isSuccess =
				(typeof result.result_code === "string" ? parseInt(result.result_code, 10) : result.result_code) === 1;
...
// 환경변수에서 테스트 모드 확인
export const isTestMode = process.env.NODE_ENV === "development" || process.env.SMS_TEST_MODE === "true";

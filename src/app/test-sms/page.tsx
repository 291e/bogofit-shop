		...
		<div className="container mx-auto py-8 max-w-2xl">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						SMS 발송 기능을 테스트해보세요. 테스트 모드(Y)로 설정되어 실제 발송되지 않습니다.
					</CardDescription>
				</CardHeader>
				...
				<CardContent className="space-y-6">
					...
					<div className="flex gap-2">
						<Button onClick={checkRemain} disabled={loading} variant="outline" className="w-full">
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
							<div>
								...
								<Input
									...
									required
								/>
								<p className="text-xs text-gray-500 mt-1">ALIGO에 등록된 발신번호만 사용 가능</p>
							</div>
							...
							<div>
								...
								<Input
									...
									required
								/>
								<p className="text-xs text-gray-500 mt-1">여러 번호는 쉼표로 구분</p>
							</div>
						</div>
						...
						<Alert>
							<AlertDescription>
								🧪 <strong>테스트 모드</strong>: 실제 문자는 발송되지 않고 API 테스트만 진행됩니다.
							</AlertDescription>
						</Alert>
									...
									<div className="mt-4 space-y-4">
										...
										{result.data.msg_id && (
											<div className="space-y-2">
												<h4 className="font-semibold text-green-700">📨 발송 결과</h4>
												<p>
													<strong>메시지 ID:</strong> {result.data.msg_id}
												</p>
												<p>
													<strong>성공 건수:</strong> {result.data.success_cnt || 0}건
												</p>
												<p>
													<strong>실패 건수:</strong> {result.data.error_cnt || 0}건
												</p>
												<p>
													<strong>메시지 타입:</strong> {result.data.msg_type || "-"}
												</p>
											</div>
										)}
										...
										{(result.data.SMS_CNT !== undefined ||
											...
											<div>
												<h4 className="font-semibold text-blue-700 mb-3">📊 잔여 포인트</h4>
												<div className="grid grid-cols-3 gap-4 p-4 bg-blue-50 rounded-lg">
													...
													<div className="text-center">
														<p className="text-sm text-gray-600">MMS (그림문자)</p>
													...
													result.data.MMS_CNT === 0 && (
														<div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
															<p className="text-yellow-800 text-sm">
																⚠️ <strong>포인트 부족:</strong> ALIGO 사이트에서
																포인트를 충전해주세요.
															</p>
														</div>
													)}
